...
fun Route.delete(db: R2dbc, dao: DAOFacade, hashFunction: (String) -> String) {
    ...
    post<KweetDelete> {
        db.withTransaction {
            ...
            val date = post["date"]?.toLongOrNull() ?: return@withTransaction call.redirect(ViewKweet(it.id))
            val code = post["code"] ?: return@withTransaction call.redirect(ViewKweet(it.id))
